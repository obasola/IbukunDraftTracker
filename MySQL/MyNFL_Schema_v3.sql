...
CREATE TABLE IF NOT EXISTS `MyNFL`.`Player` (
  ...
  `productionScore` INT NULL DEFAULT NULL,
  `athleticScore` INT NULL DEFAULT NULL,
  `totalScore` INT NULL DEFAULT NULL,   
  `combineScore` FLOAT NULL DEFAULT NULL, 
  `pickId` INT NULL DEFAULT NULL,
  PRIMARY KEY (`id`),
  INDEX `fk_Player_Pick_idx` (`pickId` ASC) VISIBLE,
